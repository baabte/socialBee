angular.module('touterbee').service('commonService',['$http','bbConfig','$state','localStorageService','$rootScope','$q',function commonService($http,bbConfig,$state,localStorageService,$rootScope,$q) {
...
// };
	...
	if(!$rootScope.userinfo){ //whenever the user refresh the page it will check the credential variable 
		if(localStorageService.get('logDatas')){ // then it will chack the local storage for neccessary datas
			...
			if(angular.equals(angular.fromJson(JSON.parse(request.responseText)),"error")||angular.equals(angular.fromJson(JSON.parse(request.responseText)),"failed")){ //if the user data not present it active user log it will push to logout
			localStorageService.set('logDatas','{}');//resetting the userinfo before logout 
